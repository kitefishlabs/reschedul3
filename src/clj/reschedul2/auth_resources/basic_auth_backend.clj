(ns reschedul2.auth-resources.basic-auth-backend
...
(defn get-user-info
  "The username and email values are stored in citext fields in Postgres thus
   ...
   for a match."
  [identifier]
  (let [registered-user-username (db/stringify-id (db/get-registered-user-details-by-username identifier))
        registered-user-email    (db/stringify-id (db/get-registered-user-details-by-email identifier))
